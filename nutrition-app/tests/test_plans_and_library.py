def test_plan_goal_and_validate(client):
    ...
    plan = client.post('/plans/', json={'date': '2026-07-10', 'name': 'Dia test'})
    ...
    validation = client.get(f'/plans/{plan_id}/validate')
...
def test_library_one_click_add_to_plan(client):
    ...
    plan = client.post('/plans/', json={'date': '2026-07-11'})
    assert plan.status_code == 201
    plan_id = plan.json()['id']

    applied = client.post(f'/plans/{plan_id}/slots/1/from-library/{template_id}')
