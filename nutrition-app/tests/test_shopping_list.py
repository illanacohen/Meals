...
def test_shopping_list_endpoint_weekly(client):
    # Day 1
    p1 = client.post('/plans/', json={'date': '2026-07-13'})
    assert p1.status_code == 201
    id1 = p1.json()['id']
    r = client.post(
        f'/plans/{id1}/slots/2/meals',
        json={
            'name': 'Almuerzo',
            'calories': 500,
            'protein': 40,
    ...
    # Day 2
    p2 = client.post('/plans/', json={'date': '2026-07-14'})
    assert p2.status_code == 201
    id2 = p2.json()['id']
    r = client.post(
        f'/plans/{id2}/slots/4/meals',
        json={
            'name': 'Cena',
            'calories': 400,
            'protein': 35,
...
def test_shopping_list_week_shortcut(client):
    plan = client.post('/plans/', json={'date': '2026-07-20'})
    assert plan.status_code == 201
    plan_id = plan.json()['id']
    client.post(
        f'/plans/{plan_id}/slots/1/meals',
        json={
            'name': 'Desayuno',
            'calories': 200,
            'protein': 12,
